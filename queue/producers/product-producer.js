...
const getListProductBySellerId = async (sellerId) => {
  let connection;
  let channel;
  try {
    connection = await amqp.connect(process.env.RABBITMQ_URL);
    ...
    return new Promise((resolve, reject) => {
      ...
      channel.sendToQueue(queue, Buffer.from(JSON.stringify({ sellerId })), {
        correlationId: correlationId,
        replyTo: replyQueue,
      });
    });
  } catch (error) {
    console.error("Error in getListProductBySellerIdProducer:", error);
    if (channel) await channel.close();
    if (connection) await connection.close();
    throw error;
  }
